...
fn fresh_solution(number: i32) -> i32 {
    ...
    let number = if let Some(n) = number.checked_abs() {
        n
    } else {
        return 0;
    };
    ...
    for i in 1..=max_10_pow {
        ...
        let add = if let Some(n) = single_digit.checked_mul(10_i32.pow(max_10_pow + 1 - i)) {
            n
        } else {
            return 0;
        };
        reversed_number = if let Some(n) = reversed_number.checked_add(add) {
            n
        } else {
            return 0;
        };
...
mod tests {
    ...
    fn bench() {
        ...
        let with_strings = bench_times(times, || {
            solution(number);
        })
        .unwrap();
        let no_std = bench_times(times, || {
            fresh_solution(number);
        })
        .unwrap();
