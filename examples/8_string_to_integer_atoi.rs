...
fn two_letter(cleaned_s: &str) -> i32 {
    match (
        cleaned_s[0..1].chars().last(),
        cleaned_s[1..2].chars().last(),
    ) {
        (Some('-'), Some(two @ '0'..='9')) => -(two as i32 - 48),
        (Some('+'), Some(two @ '0'..='9')) => two as i32 - 48,
        (Some(one @ '0'..='9'), Some(two @ '0'..='9')) => {
            (one as i32 - 48) * 10 + (two as i32 - 48)
        }
...
fn three_or_more_letters(cleaned_s: &str) -> i32 {
    ...
    let (one, two) = (
        cleaned_s[0..1].chars().last().unwrap(),
        cleaned_s[1..2].chars().last().unwrap(),
    );
    let is_negative = one == '-';
    if !matches!((one, two), ('-' | '+', '0'..='9') | ('0'..='9', ..)) {
        return DEFAULT;
    }
...
mod tests {
    ...
    fn solution_test() {
        ...
        assert_eq!(
            solution("           -869765436578966786467535424315325465487607698656321 lokcrtssrt"),
            min
        );
    }
    ...
    fn not_my_solution_test() {
        ...
        assert_eq!(
            my_atoi("           -869765436578966786467535424315325465487607698656321 lokcrtssrt"),
            min
        );
    }
    ...
    fn bench() {
        ...
        let my_long_sol = bench_times(number_of_iterations, || {
            solution(&s);
        });

        let mut s = String::from_iter(vec![filler_value; fillers_number].into_iter());
        s.insert(fillers_number / 2, '-');
        let short_func_sol = bench_times(number_of_iterations, || {
            my_atoi(&s);
        });
